...
excluded_addr_count = 0

def is_excluded_address(ip_address):
    global excluded_addr_count
    excluded_prefixes = ['127.0.0.', '[::]', '91.198.115.', '162.218.65.', '209.222.252.']
    for prefix in excluded_prefixes:
        if ip_address.startswith(prefix):
            excluded_addr_count += 1
            return True
    return False

def parse_log_file(log_file_path):
    ip_pattern = r'Peer disconnected: ((?:[0-9]{1,3}\.){3}[0-9]{1,3}|(?:[0-9a-zA-Z]+\.onion)):\d+'  # IPv4 and Onion
...
def main():
    log_file_path = '../bitcoin-data/peer_disconnect_addr.log'
    count_file_path = '../bitcoin-data/analysis/detail/disconnect_count.log'
    peer_file_path = '../bitcoin-data/analysis/suspect_addr/disconnect_sus.log'
    ...
    print(f"Excluded addresses: {excluded_addr_count}")
